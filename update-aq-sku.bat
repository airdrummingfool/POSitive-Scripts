...
set INVNO=null
for /f "delims=" %%a in ('call util\sqlcmdwrapper.bat -d %DB% -Q "SET NOCOUNT ON; SELECT TOP 1 BAR_INVNO FROM BARCODES WHERE BAR_ID = 1 AND BAR_BARCODE='%PrimarySKU%';" -h -1 -W') do @set INVNO=%%a
IF "%INVNO%" == "null" (echo Couldn't find item! && PAUSE && exit /b)

set DESC=null
for /f "delims=" %%a in ('call util\sqlcmdwrapper.bat -d %DB% -Q "SET NOCOUNT ON; SELECT TOP 1 BAR_DESCRIPTION FROM BARCODES WHERE BAR_ID = 1 AND BAR_INVNO='%INVNO%';" -h -1 -W') do @set DESC=%%a
